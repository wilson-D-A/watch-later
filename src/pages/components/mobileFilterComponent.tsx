				...
				<div className='grow sm:grow-0 rounded  border-border border-2 w-auto h-auto flex-1 min-h-0 overflow-y-auto scrollbar-none '>
					<div className='flex  mx-5 my-2 justify-between '>
						<h2
							...
							className={`${isMoreConcepts ? 'bg-accent text-zinc-900' : 'bg-accent/40 text-zinc-300'} text-sm grow   cursor-pointer rounded-l-lg px-4 py-1 `}
						>
							concepts
						</h2>
						<div
							onClick={() => setIsMoreConcepts(false)}
							className={`${!isMoreConcepts ? 'bg-accent *:text-zinc-900 ' : 'bg-accent/40  '} flex gap-5 p-1  rounded-r-lg  cursor-pointer  px-2 py-1  `}
						>
